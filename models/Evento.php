...
class Evento extends ActiveRecord
{
    ...
    public function validar()
    {
        ...
        if (strlen($this->nombre) > 120) {
            self::$alertas['error'][] = 'El Nombre no puede tener más de 120 caracteres';
        }
        if (!$this->descripcion) {
            self::$alertas['error'][] = 'La descripción es Obligatoria';
        }
        if (strlen($this->descripcion) > 65000) {
            self::$alertas['error'][] = 'La descripción no puede tener más de 65000 caracteres';
        }
